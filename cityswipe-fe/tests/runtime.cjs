...
async function main() {
  ...
  const actions=load('app/actions.ts',{
    ...
    '@ai-sdk/google':{google:()=>({})},ai:{streamText:async()=>{throw Error('Provider offline');},generateText:async()=>{if(generationFailure)throw Error('Provider offline');return {text:'Welcome to New York.'};}},
  ...
  const reply=await actions.streamFlirtatiousConversation('New York','United States',[]);assert.equal(reply.newMessage,'Welcome to New York.');generationFailure=true;await assert.rejects(actions.streamFlirtatiousConversation('New York','United States',[]),/Unable to contact/);console.log('PASS chat returns complete replies and reports provider failures');
  await actions.streamConversation([]);await new Promise(resolve=>setImmediate(resolve));assert.match(streamError.message,/Unable to contact/);console.log('PASS AI failures close the client stream with an error');
  ...
  console.log('10 regression checks passed');
}
