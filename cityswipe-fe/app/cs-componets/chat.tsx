...
import { streamConversation, getConversationHistory, Message, streamFlirtatiousConversation, makeItinerary } from "../actions";
...
export default function Chat({matches}: any) {
    ...
    const startChat = async () => {
        ...
        if (wantsItinerary != true) {
            const { messages, newMessage, type } = await streamFlirtatiousConversation(split == undefined ? "" : split[0], split == undefined ? "" : split[1], [
                ...conversation,
                { role: "user", content: input, type: "message" },
            ]);
    ...
    return (
        ...
        <div className="relative hidden sm:flex px-[10%]  text-[14px] flex-col place-content-center place-items-center w-full h-[100dvh]">
            ...
            <div className="absolute border-b border-primary/20 top-0 w-full h-[6%] flex justify-between place-items-center p-5">
                
                {selectedMatch == '' ? (
                    <h2 className="select-none">You are a <span className="text-green-400 font-bold">{usermatches?.[0]?.compatibility}%</span> match with <strong>{usermatches?.[0]?.city}</strong>!</h2>
                ) : (
                    <h2 className="select-none">You are a <span className="text-green-400 font-bold">{selectedCompatibility}%</span>  match with <strong>{selectedMatch}</strong>!</h2>
                )}
            </div>
                    ...
                    <div className="flex  flex-col select-none gap-1 place-items-center w-full">
                        ...
                        {selectedMatch == '' ? (
                            <>
                                ...
                                <div className="w-full flex justify-between gap-4 place-items-start">
                                ...
                                className="font-bold text-2xl">
                                    <SparklesText className="text-3xl" colors={{ first: '#22d3ee', second: '#4ade80' }} text={`You are a ${usermatches?.[0]?.compatibility}% match with ${usermatches?.[0]?.city}!`} />
                                </motion.span>

                                <motion.p className="w-full text-[12px]">
                                    <span className="font-bold">Bio:</span> {usermatches?.[0]?.description}
                                </motion.p>
                                ...
                                className="w-[60px] h-[60px] rounded-full flex place-items-end place-content-end">
                                        <Image className="rounded-full h-full  w-full object-cover" src={usermatches?.[0]?.illustration || placeholderimg} sizes="100%" width={30} height={30} alt="" />
                                </motion.div>
                                ...
                                <div className="my-2">
                                    ...
                                    {userdata?.[0]?.cons?.slice(0, 3).map((con: any, index: number) => (
                                        <motion.div 
                                        initial={{ opacity: 0, y: 100 }}
                                        animate={{ opacity: 1, y: 0 }}
                                        transition={{ duration: 0.75 }}
                            ...
                            <div className="w-full flex justify-between gap-4 place-items-start">
                                ...
                                transition={{ duration: 0.75 }}
                                className="font-bold text-2xl">
                                    <SparklesText className="text-3xl" colors={{ first: '#22d3ee', second: '#4ade80' }} text={`You are a ${selectedCompatibility}% match with ${selectedMatch}!`} />
                                </motion.span>
            ...
            {conversation.length > 0 && (
                <>
                    <div id="chat-container" className=" flex flex-col w-full h-[80vh] no-scrollbar overflow-y-scroll ">
                        {conversation.map((message, index) => (
                            <div key={index} className={`min-w-[20%] max-w-[70%] h-max flex ${message.role === 'user' ? 'place-self-end' : 'place-self-start'}`}>
                               
                                {message.role === 'assistant' && (
                                    <Image src={chatImg || placeholderimg} sizes="100%" width={30} height={30} className="object-cover w-[30px] h-[30px] rounded-full my-3" alt="" />
                                )}
