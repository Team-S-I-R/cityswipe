...
export default function QuizClient({ clerkdata }: any) {
  ...
  const handleGemini = async () => {
    ...
    const defaults = ["United States", "Any Budget", "Open to All Experiences", "Open to All Forms of Travel", "Open to All"];
    const answers = responses.map((response, index) => response.trim() || defaults[index]);
    ...
    try {
      ...
      await setDestinationSet({
        ...
        allCards: [...destinations],
        responses: answers,
      });

      await addQuestions(answers);
